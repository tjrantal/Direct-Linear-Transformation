...
addpath('functions');
global eqsToUse
eqsToUse = 16;  %Use this many equations in optimised DLT (between 12 and 16, affects lens distortion correction)
calibrationDigitized = 1; %=0 if you haven't digitized yet, =1 if you have
dataSaveName = 'digitized_30_points.mat';
...
frameName = 'sampleFigs/gopro_calibFrame_30_points.txt';
calibrationFrame = dlmread('sampleFigs/gopro_calibFrame_30_points.txt','\t',1,1);

%Read titles
fh = fopen(frameName,'r');
fgetl(fh);  %Discard header
temp = fgetl(fh);  %Discard header
titles = {};
while temp ~=-1
    splitLine = strsplit(temp,'\t');
    titles = [titles; splitLine{1}];
    temp = fgetl(fh);  %Discard header
end
fclose(fh);

imageNames = {'sampleFigs/GOPR0004_1607335405391.JPG','sampleFigs/GOPR0005_1607335405391.JPG'};
...
if calibrationDigitized == 1
	...
	for i = 1:length(imageNames)
        ...
        if isfield(cam(i),'digitizedCoordinates') && ~isempty(cam(i).digitizedCoordinates)
            ...
            cam(i).digitizedCoordinates = digitizeCalibration(cam(i).image,calibrationFrame,cam(i).digitizedCoordinates,titles);
        else
            cam(i).digitizedCoordinates = digitizeCalibration(cam(i).image,calibrationFrame,[],titles);
        end
	end
