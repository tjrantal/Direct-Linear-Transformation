...
dataSaveName = 'undistorted_digitized.mat';
...
calibFile = 'sampleFigs/calibFrame.txt';
calibrationFrame = dlmread(calibFile,'\t',1,1);
markerNames = {};
fh = fopen(calibFile,'r');
test = fgetl(fh);
test = fgetl(fh);
while test ~= -1
    split = strsplit(test,'\t');
    markerNames = [markerNames; split{1}];
    test = fgetl(fh);
end
fclose(fh);
% keyboard;
...
if calibrationDigitized == 1
	...
	for i = 1:length(imageNames)
        ...
        if isfield(cam(i),'digitizedCoordinates') && ~isempty(cam(i).digitizedCoordinates)
            ...
            cam(i).digitizedCoordinates = digitizeCalibration(cam(i).image,calibrationFrame,cam(i).digitizedCoordinates,markerNames);
        else
            cam(i).digitizedCoordinates = digitizeCalibration(cam(i).image,calibrationFrame, [],markerNames);
        end
	end
